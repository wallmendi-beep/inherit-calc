...
**포함되어야 할 내용:**
- 피상속인별 상속지분 분기 시각화 (마인드맵/flow 뷰)
- 좌측 사이드패널 사건 보고서
- 스마트 가이드 네비게이션 대상 탭
- 각 상속인 노드 (관계, 이름, 통분지분, 분기정보)

**포함되면 안 되는 내용:**
- 데이터 직접 입력 UI
- 취득경로 표
- 상속권 없는 사람의 그래프 노드 표시
- 사건 보고서 안의 중복 설명/노드 해설

---

#### 2.2.1 사건 검토 레이아웃 원칙

`tree` 탭은 `input` 탭과 같은 좌측 사이드패널 구조를 사용하되, 패널의 역할이 다르다.

| 탭 | 좌측 사이드패널 | 메인 영역 |
|---|---|---|
| `input` | 상속가계도 | 데이터 입력/수정 |
| `tree` | 사건 보고서 | 상속지분 마인드맵 |

사건 보고서는 선택한 노드가 속한 **하나의 상속사건**만 설명한다.  
마인드맵은 분기 구조를 넓게 보여주고, 보고서는 판정 근거를 좁은 세로 문서로 보여준다.

#### 2.2.2 사건 보고서 표시 규칙

상단은 사이드패널 문서임을 알 수 있도록 `사건 보고서` 제목을 두고, 기본정보는 두 줄로 압축한다.
...
보고서 본문은 다음 3개 섹션만 둔다.

1. `사건 관계인`
   - 클릭한 사건에 관련된 사람만 표시한다.
   - 표 컬럼은 `성명 / 관계 / 지분 / 가감산 요소`로 제한한다.
   - 사건 관계인 표의 지분은 `2/165`처럼 분수로 표시한다.
   - `판정` 컬럼은 두지 않는다.
   - 상속권이 없는 사람은 그래프 노드에서는 숨기되, 보고서에는 `가감산 요소: 상속권 없음`으로 표시한다.

2. `사건 요약`
   - `상속권 없음`, `가감산 요소`, `재상속·대습상속`으로 분리한다.
   - 한 사람이 여러 사유를 가지면 해당 섹션에 각각 표시한다.
   - 입력된 날짜가 있으면 문구에 반영한다.

3. `판정 요약`
   - 표와 사건 요약만으로 부족한 경우에만 짧게 설명한다.
   - 노드 설명, 원자료 전체 목록, 취득경로 요약은 넣지 않는다.

#### 2.2.3 사건 요약 문구 규칙

데이터 입력에서 수집한 정보를 최대한 사용하되, 문장은 짧게 유지한다.
...
날짜가 없으면 `날짜 미상`을 남발하지 않고 사유만 표시한다.
...
| 탭 | 사이드바 상태 |
|---|---|
| `input` | 상속가계도 사이드패널 기본 열림 |
| `tree` | 사건 보고서 사이드패널 기본 열림 |
...
| 2026-05-04 | 사건 검토 탭의 좌측 사건 보고서 사이드패널 원칙, 보고서 섹션, 적용법 표기, 사건 요약 문구 규칙 추가 |
