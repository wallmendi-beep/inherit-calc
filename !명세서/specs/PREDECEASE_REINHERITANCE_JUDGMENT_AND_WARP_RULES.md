...
|---|---|---|---|
| 대습상속 미확정 | 선사망자가 직접 행으로 표시되는 부모 탭 | 데이터 입력 | 해당 행에서 대습상속인 추가 또는 없음 확정을 처리 |
| 후속 상속 미확정 - 단일 | 사망자 본인 사건 | 사건 검토 | 본인 재상속 사건에서 후속 상속인 입력/확정 |
| 후속 상속 미확정 - 복수 | contextNode 사건 | 사건 검토 | 같은 사건 내 여러 후사망자를 한 번에 확인 |

대습상속 미확정 문구의 사건명은 `contextNode`를 기준으로 표시한다. 예를 들어 `구수명 사건의 윤종옥 계통 선사망자`라고 표시하더라도, 클릭 이동은 구수명 탭이 아니라 윤우경/윤우철 행이 실제로 렌더링되는 윤종옥 탭으로 가야 한다.

서로 다른 직접 부모 계통의 선사망자를 한 카드에 섞으면 하이라이트 대상이 한 화면에 존재하지 않을 수 있으므로, 그룹 키는 `contextNode + 직접 부모` 단위로 분리한다.
...
| 2026-05-03 | v4.82 — 대습상속 미확정 가이드는 직접 부모 탭으로 이동하도록 보정 |
