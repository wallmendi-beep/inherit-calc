...
작성일: 2026-05-01  
최종 업데이트: 2026-05-06 (v4.87 - 선사망·후속없음 분배대상 제외 기준 반영)  
...
동일인이 여러 사건 단계에 반복 등장하더라도 그 자체를 `duplicate` 상속 제외로 처리하지 않는다. 같은 `personId` 반복 등장은 하위 전개 중단으로 순환만 막고, 해당 사건에서의 상속인 포함 여부는 현재 `contextDate` 기준으로 다시 판정한다.
...
### 3.4 선사망·후속없음 분배대상 제외

현재 사건 기준일보다 먼저 사망한 인물이 유효한 대습/후속 상속인을 갖지 않으면 분배대상에서 제외한다.

```text
node.deathDate < contextDate
validSubstituteHeirs.length === 0
```

위 조건이면 해당 인물은 분배분모에 들어가지 않는다. 이는 `isExcluded: true`, `exclusionOption: "predeceased"`의 표시 상태뿐 아니라 실제 하위 상속인 유효성 판정까지 함께 본다.

예: 김혁조 사건의 김명남 `1/15` 경로

- 포함: 윤종옥, 윤우영, 윤정희, 윤방자, 윤숙자, 윤우성
- 제외: 윤우경, 윤우철

윤우경·윤우철은 김명남 사건 기준 선사망했고 유효한 대습/후속 상속인이 없으므로 분모에서 제외한다.
...
| 2026-05-06 | v4.87 — 동일인 반복 등장은 상속 제외하지 않고, 선사망·후속없음 인물은 분배분모에서 제외하는 기준 반영 |
