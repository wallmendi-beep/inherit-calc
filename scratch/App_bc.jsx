...
function App() {
  ...
  const handleGuideClick = (g) => {
    ...
    // ?슚 ?됰슧???ㅽ겕濡?諛⑹? 諛??덉쟾???붾㈃ ?대룞 濡쒖쭅
    setTimeout(() => {
      const el = document.getElementById(`node-${g.id}`);
      if (el) {
        // 遺紐???쑝濡??뚰봽?섏뿬 ?대떦 移대뱶媛 ?붾㈃??議댁옱?섎뒗 寃쎌슦 -> ?대떦 移대뱶濡??ㅽ겕濡?諛??섏씠?쇱씠???④낵
        ...
        // 蹂몄씤 ??쑝濡??뚰봽?섏뿬 ?먯떊??移대뱶媛 ?녿뒗 寃쎌슦 (?먮뒗 ?붿냼瑜?李얠? 紐삵븳 寃쎌슦) -> ?붾㈃ 留??꾨줈 遺?쒕읇寃??ㅽ겕濡?        window.scrollTo({ top: 0, behavior: 'smooth' });
      }
    }, 150);
  };
  ...
  const tabData = [
    { id: 'input', label: '?곗씠???낅젰', icon: <IconFileText className="w-4 h-4"/>, style: { activeBorder: 'border-[#37352f]', activeText: 'text-[#37352f] dark:text-neutral-100', inactiveBg: 'bg-transparent', inactiveBorder: 'border-transparent', inactiveText: 'text-[#9b9a97]' } },
    { id: 'tree', label: '媛怨꾨룄', icon: <IconNetwork className="w-4 h-4"/>, style: { activeBorder: 'border-[#37352f]', activeText: 'text-[#37352f] dark:text-neutral-100', inactiveBg: 'bg-transparent', inactiveBorder: 'border-transparent', inactiveText: 'text-[#9b9a97]' } },
    { id: 'calc', label: '怨꾩궛??, icon: <IconTable className="w-4 h-4"/>, style: { activeBorder: 'border-[#37352f]', activeText: 'text-[#37352f] dark:text-neutral-100', inactiveBg: 'bg-transparent', inactiveBorder: 'border-transparent', inactiveText: 'text-[#9b9a97]' } },
    { id: 'result', label: '怨꾩궛寃곌낵', icon: <IconCalculator className="w-4 h-4"/>, style: { activeBorder: 'border-[#37352f]', activeText: 'text-[#37352f] dark:text-neutral-100', inactiveBg: 'bg-transparent', inactiveBorder: 'border-transparent', inactiveText: 'text-[#9b9a97]' } },
    { id: 'summary', label: '踰뺤젙 ?곸냽遺??붿빟??, icon: <IconList className="w-4 h-4"/>,
    ...
    { id: 'amount', label: '援ъ껜???곸냽遺?怨꾩궛', icon: <IconCalculator className="w-4 h-4 text-green-600"/>,
  ...
  const handlePrint = () => { if (activeTab === 'input') { alert('蹂닿퀬?????곗텧?댁뿭, 吏遺꾩슂?? ?곸냽湲덉븸) 以??섎굹瑜??좏깮?????몄뇙?댁＜?몄슂.'); return; } const tabNames = { input: '媛怨꾨룄', calc: '?곸냽吏遺??곗텧?댁뿭', summary: '踰뺤젙?곸냽遺??붿빟??, amount: '援ъ껜?곸긽?띾텇_寃곌낵' }; const currentTabName = tabNames[activeTab] || '蹂닿퀬??; const safeCaseNo = (tree.caseNo || '?ш굔踰덊샇?놁쓬').replace(/[^a-zA-Z0-9媛-??-]/g, ''); const safeName = (tree.name || '?쇱긽?띿씤?놁쓬').replace(/[^a-zA-Z0-9媛-??-]/g, ''); const printFileName = `${safeCaseNo}_${safeName}_${currentTabName}_${new Date().toISOString().slice(0, 10)}`; const originalTitle = document.title; document.title = printFileName; window.print(); document.title = originalTitle; };
    ...
    reader.onload = (ev) => { 
      try { 
        ...
        const sanitizeNode = (n, parentDate) => {
          ...
          // 1. ?좎궗留앹옄 '?곸냽?ш린(renounce)' 李뚭볼湲?移섏쑀
          if (isExcluded && ['renounce', 'no_heir'].includes(exclusionOption) && isPredeceased) {
              exclusionOption = 'predeceased';
          }

          // 2. ?좊쾿(1991~) ?곸슜 ??吏곴퀎鍮꾩냽??臾댁쓽誘명븳 ?쇱씤/?ы샎?쇱옄 利앸컻
          const era = getLawEra(refDate);
          const isSpouseType = ['wife', 'husband', 'spouse', '泥?, '?⑦렪', '諛곗슦??].includes(n.relation);
      ...
      } catch { alert('?뚯씪???쎈뒗 以??ㅻ쪟媛 諛쒖깮?덉뒿?덈떎.'); } 
    }; 
...
[JSON ?ㅽ궎留??묒떇]
{
  ...
  const handleExcelExport = () => { const rows = [['?ш굔踰덊샇', tree.caseNo || ''], ['?쇱긽?띿씤', tree.name || ''], ['?щ쭩?쇱옄', tree.deathDate || ''], [''], ['?곸냽??, '愿怨?, '吏遺?遺꾩옄)', '吏遺?遺꾨え)', '?듬텇 吏遺?遺꾩옄)', '?듬텇 吏遺?遺꾨え)']]; finalShares.direct.forEach(f => rows.push([f.name, getRelStr(f.relation, tree.deathDate) || f.relation, f.n, f.d, f.un, f.ud])); (finalShares.subGroups || []).forEach(g => { rows.push(['', `??怨듬룞?곸냽??以?[${g.ancestor?.name || ''}]?(?? ${formatKorDate(g.ancestor?.deathDate)} ?щ쭩?섏??쇰?濡??곸냽??, '', '', '', '']); g.shares.forEach(f => rows.push([f.name, getRelStr(f.relation, tree.deathDate) || f.relation, f.n, f.d, f.un, f.ud])); }); const csv = '\uFEFF' + rows.map(r => r.map(c => `"${String(c).replace(/"/g, '""')}"`).join(',')).join('\n'); const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `?곸냽吏遺?${(tree.name || '?쇱긽?띿씤?놁쓬').replace(/[^a-zA-Z0-9媛-??-]/g, '')}_${new Date().toISOString().slice(0,10)}.csv`; a.click(); URL.revokeObjectURL(url); };
    ...
    <>
            ...
            <div className="flex flex-col gap-3 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
              {!isNavigatorRolledUp && (
                <div className="text-[13px] font-bold text-[#504f4c] dark:text-neutral-300 pointer-events-none animate-in fade-in slide-in-from-top-1 duration-200">
                  ...
                  {activeTab === 'input' && smartGuides.filter(g => g.type === 'mandatory').map((g, i) => (<button key={`m-${i}`} onMouseDown={(e) => e.stopPropagation()} onClick={() => handleGuideClick(g)} className="w-full mt-2 text-left flex items-start gap-2 bg-blue-50/60 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200/60 dark:border-blue-800/30 hover:bg-blue-100/80 transition-all group pointer-events-auto shadow-sm"><span className="mt-0.5 text-blue-600 group-hover:scale-125 transition-transform">?몛</span><span className="flex-1 leading-snug text-[#37352f] dark:text-neutral-200 font-bold">{g.text}</span></button>))}
      ...
      <div className="bg-white dark:bg-neutral-800 border-b border-[#e9e9e7] dark:border-neutral-700 h-[54px] sticky top-0 z-50 no-print w-full flex justify-start transition-all duration-300 shadow-sm overflow-hidden">
        <div className="w-[1080px] min-w-[1080px] shrink-0 px-6 flex items-center justify-between h-full flex-nowrap">
          <div className="flex items-center gap-3 flex-nowrap shrink-0">
            <button onClick={() => setSidebarOpen(v => !v)} className={`w-7 h-7 flex flex-col justify-center items-center rounded-md gap-1 transition-all no-print ${sidebarOpen ? 'bg-[#f0f0ee] dark:bg-neutral-700 text-[#2383e2] dark:text-blue-400' : 'text-[#787774] dark:text-neutral-400 hover:bg-[#efefed] dark:hover:bg-neutral-700'}`} title={sidebarOpen ? '媛怨꾨룄 ?⑤꼸 ?リ린' : '媛怨꾨룄 ?⑤꼸 ?닿린'}><span className="w-3.5 h-0.5 bg-current rounded-full transition-all" /><span className="w-3.5 h-0.5 bg-current rounded-full transition-all" /><span className="w-3.5 h-0.5 bg-current rounded-full transition-all" /></button>
            <div className="flex items-center gap-2 whitespace-nowrap shrink-0 overflow-visible"><div className="flex items-center text-[#37352f] dark:text-neutral-100 font-bold text-[18px] tracking-tight whitespace-nowrap shrink-0"><IconCalculator className="w-5 h-5 mr-1.5 text-[#787774] dark:text-neutral-400 shrink-0" />?곸냽吏遺?怨꾩궛湲?PRO <span className="ml-1.5 text-[11px] font-medium bg-[#e9e9e7] dark:bg-neutral-700 px-1.5 py-0.5 rounded text-[#787774] dark:text-neutral-400 shrink-0">v3.0.0</span></div><span className="designer-sign text-[#a3a3a3] dark:text-neutral-500 text-[14px] ml-8 whitespace-nowrap shrink-0">Designed by J.H. Lee</span></div>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            <div className="flex items-center gap-1.5 bg-[#f7f7f5] dark:bg-neutral-700 px-2.5 py-1 rounded border border-[#e9e9e7] dark:border-neutral-600 mr-2 transition-colors"><div className="min-w-[120px] flex items-center gap-1 overflow-hidden"><span className="text-[11px] font-bold text-[#787774] dark:text-neutral-400 whitespace-nowrap">?ш굔:</span><span className="text-[11px] font-bold text-[#37352f] dark:text-neutral-200 truncate">{tree.caseNo || '誘몄엯??}</span></div><div className="w-px h-2.5 bg-[#d4d4d4] dark:bg-neutral-600 mx-0.5"></div><div className="min-w-[140px] flex items-center gap-1 overflow-hidden"><span className="text-[11px] font-bold text-[#787774] dark:text-neutral-400 whitespace-nowrap">?쇱긽?띿씤:</span><span className="text-[13px] font-black text-[#0b6e99] dark:text-blue-400 truncate">{tree.name || '誘몄엯??}</span></div></div>
            <button onClick={() => { setAiTargetId('root'); setIsAiModalOpen(true); }} title="媛怨꾨룄 ?꾩껜 AI ?먮룞?낅젰" className="flex items-center justify-center w-8 h-8 shrink-0 bg-white dark:bg-neutral-800 border border-[#e9e9e7] dark:border-neutral-700 hover:bg-[#f7f7f5] dark:hover:bg-neutral-700/50 rounded-lg transition-all shadow-sm group">
              <IconSparkles size={16} className="text-neutral-400 group-hover:text-[#2383e2] transition-colors" />
            </button>
            <button onClick={() => setShowNavigator(true)} className={`flex items-center justify-center w-8 h-8 rounded-lg transition-all shadow-sm border shrink-0 mx-[10px] active:scale-95 ${hasActionItems ? 'bg-indigo-50 text-indigo-600 border-indigo-100 hover:bg-indigo-100 dark:bg-indigo-900/20 dark:text-indigo-400 dark:border-indigo-800/50 dark:hover:bg-indigo-900/40' : 'bg-white text-[#787774] border-[#e9e9e7] hover:bg-[#f7f7f5] hover:text-[#37352f] dark:bg-neutral-800 dark:border-neutral-700 dark:hover:bg-neutral-700'}`} title={hasActionItems ? "?덈줈???ㅻ쭏??媛?대뱶媛 ?덉뒿?덈떎!" : "?ㅻ쭏??媛?대뱶 ?닿린"}><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={hasActionItems ? 2.5 : 2}><circle cx="12" cy="12" r="10" /><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76" /></svg></button>
            ...
            <button onClick={redoTree} disabled={vaultState.currentIndex >= vaultState.history.length - 1} className="disabled:opacity-40 disabled:cursor-not-allowed text-[#787774] hover:text-[#37352f] hover:bg-[#efefed] px-2 py-1 rounded border border-transparent hover:border-[#d4d4d4] text-[12px] font-bold transition-colors flex items-center gap-1"><IconRedo className="w-3.5 h-3.5" /> ?ъ떎??/button>
            <div className="w-px h-3.5 bg-[#e9e9e7] dark:bg-neutral-600 mx-0.5"></div>
            <button onClick={() => setIsResetModalOpen(true)} className="text-[#787774] hover:text-[#37352f] hover:bg-[#efefed] px-2 py-1 rounded border border-transparent hover:border-[#d4d4d4] text-[12px] font-bold transition-colors flex items-center gap-1"><IconReset className="h-3.5 w-3.5" /> 珥덇린??/button>
            <label className="text-[#787774] hover:text-[#37352f] hover:bg-[#efefed] px-2 py-1 rounded border border-transparent hover:border-[#d4d4d4] text-[12px] font-bold transition-colors flex items-center gap-1 cursor-pointer"><IconFolderOpen className="h-3.5 w-3.5" /> 遺덈윭?ㅺ린<input type="file" accept=".json" onChange={loadFile} className="hidden" /></label>
            <button onClick={saveFile} className="text-[#787774] hover:text-[#37352f] hover:bg-[#efefed] px-2 py-1 rounded border border-transparent hover:border-[#d4d4d4] text-[12px] font-bold transition-colors flex items-center gap-1"><IconSave className="h-3.5 w-3.5" /> ???/button>
            <button onClick={handleExcelExport} className="text-[#787774] hover:text-[#37352f] hover:bg-[#efefed] px-2 py-1 rounded border border-transparent hover:border-[#d4d4d4] text-[12px] font-bold transition-colors flex items-center gap-1"><IconTable className="h-3.5 w-3.5" /> ?묒?</button>
            <button onClick={handlePrint} className="text-white bg-[#2383e2] hover:bg-[#0073ea] px-3 py-1 rounded text-[12px] font-bold transition-colors flex items-center gap-1 shadow-sm whitespace-nowrap ml-1 mr-1"><IconPrinter className="h-3.5 w-3.5" /> ?몄뇙</button>
          ...
          <div className="flex flex-col bg-white dark:bg-neutral-800 border-r border-[#e9e9e7] dark:border-neutral-700 overflow-hidden" style={{ width: sidebarWidth }}>
            <div className="p-2 border-b border-[#e9e9e7] dark:border-neutral-700 flex flex-col gap-1.5 shrink-0">
              <div className="flex items-center gap-1 bg-[#f7f7f5] dark:bg-neutral-900 border border-[#e9e9e7] dark:border-neutral-700 rounded px-2 py-1">
                ...
                <input type="text" placeholder="?대쫫 寃?? value={sidebarSearchQuery} onChange={e => setSidebarSearchQuery(e.target.value)} className="bg-transparent outline-none flex-1 text-[12px] text-[#37352f] dark:text-neutral-200 min-w-0" />
              ...
              <div className="flex gap-1">
                <button onClick={() => setSidebarToggleSignal(s => Math.abs(s) + 1)} className="flex-1 text-[10px] text-[#787774] dark:text-neutral-400 hover:text-[#37352f] hover:bg-[#f0f0ee] dark:hover:bg-neutral-700 py-0.5 rounded transition-colors font-bold">紐⑤몢 ?쇱튂湲?/button>
                <button onClick={() => setSidebarToggleSignal(s => -(Math.abs(s) + 1))} className="flex-1 text-[10px] text-[#787774] dark:text-neutral-400 hover:text-[#37352f] hover:bg-[#f0f0ee] dark:hover:bg-neutral-700 py-0.5 rounded transition-colors font-bold">紐⑤몢 ?묎린</button>
              </div>
            </div>
                ...
                return (
                  <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-400 flex flex-col flex-1">
                    <div className="bg-white dark:bg-neutral-800/20 border border-[#e9e9e7] dark:border-neutral-700 rounded-lg px-6 py-3 flex items-center gap-6 transition-colors shadow-sm">
                      <div className="flex items-center gap-2 shrink-0 border-r border-[#f1f1ef] dark:border-neutral-700/50 pr-6 py-1"><div className="w-1.5 h-1.5 rounded-full bg-neutral-300 dark:bg-neutral-600" /><span className="text-[12px] font-bold text-[#787774] dark:text-neutral-400 uppercase tracking-widest">湲곕낯?뺣낫</span></div>
                      <div className="flex flex-1 items-center gap-5 overflow-x-auto no-scrollbar">
                        <div className="shrink-0 flex items-center gap-2"><label className="text-[12px] text-[#787774] dark:text-neutral-400 font-bold whitespace-nowrap">?ш굔踰덊샇</label><input type="text" onKeyDown={handleKeyDown} value={tree.caseNo || ''} onChange={e=>handleRootUpdate('caseNo',e.target.value)} className="w-32 border border-[#e9e9e7] dark:border-neutral-700 rounded px-2.5 py-1.5 text-[14px] font-medium text-[#37352f] dark:text-neutral-200 outline-none transition-all bg-transparent focus:bg-neutral-50 dark:focus:bg-neutral-800" placeholder="踰덊샇 ?낅젰" /></div>
                        <div className="shrink-0 flex items-center gap-2"><label className="text-[12px] text-[#787774] dark:text-neutral-400 font-bold whitespace-nowrap">?깅챸</label><input type="text" onKeyDown={handleKeyDown} value={tree.name || ''} onChange={e=>handleRootUpdate('name',e.target.value)} className="w-28 border border-[#e9e9e7] dark:border-neutral-700 rounded px-2.5 py-1.5 text-[14px] font-bold text-[#37352f] dark:text-neutral-100 outline-none transition-all bg-transparent focus:bg-neutral-50 dark:focus:bg-neutral-800" placeholder="?대쫫" /></div>
                        <div className="shrink-0 flex items-center gap-2"><label className="text-[12px] text-[#787774] dark:text-neutral-400 font-bold whitespace-nowrap">?щ쭩?쇱옄</label><DateInput value={tree.deathDate || ''} onKeyDown={handleKeyDown} onChange={v=>handleRootUpdate('deathDate', v)} className="w-28 border border-[#e9e9e7] dark:border-neutral-700 rounded px-2.5 py-1.5 text-[14px] font-medium text-[#37352f] dark:text-neutral-200 outline-none transition-all bg-transparent focus:bg-neutral-50 dark:focus:bg-neutral-800" /></div>
                        {getLawEra(tree.deathDate) !== '1991' && <div className="shrink-0 flex items-center gap-2"><label className="text-[12px] text-[#787774] dark:text-neutral-400 font-bold whitespace-nowrap">?몄＜</label><input type="checkbox" disabled={!isRootNode} checked={isRootNode ? tree.isHoju !== false : false} onChange={e=>handleRootUpdate('isHoju', e.target.checked)} className="w-3.5 h-3.5 cursor-pointer accent-neutral-500" /></div>}
                        <div className="shrink-0 flex items-center gap-2"><label className="text-[12px] text-[#787774] dark:text-neutral-400 font-bold whitespace-nowrap">?곸냽??吏遺?/label><div className="flex items-center bg-transparent rounded border border-[#e9e9e7] dark:border-neutral-700 px-2 py-1 gap-1"><input type="number" min="1" value={tree.shareD || 1} onChange={e=>handleRootUpdate('shareD', Math.max(1, parseInt(e.target.value)||1))} className="w-10 bg-transparent text-[14px] text-center font-medium text-[#37352f] outline-none dark:text-neutral-200" title="遺꾨え" /><span className="text-[#787774] dark:text-neutral-500 text-[12px] font-medium mx-0.5">/</span><input type="number" min="1" max={tree.shareD || 1} value={tree.shareN || 1} onChange={e=>handleRootUpdate('shareN', Math.min(tree.shareD||1, Math.max(1, parseInt(e.target.value)||1)))} className="w-10 bg-transparent text-[14px] text-center font-medium text-[#37352f] outline-none dark:text-neutral-200" title="遺꾩옄" /></div></div>
                      </div>
                    </div>
                    <div className="transition-colors flex-1 flex flex-col">
                      <div className="relative transition-all duration-300 flex-1 bg-white dark:bg-neutral-800 border border-[#e9e9e7] dark:border-neutral-700/50 rounded-xl">
                        <div className="flex items-stretch px-6 py-3 border-b border-[#f1f1ef] dark:border-neutral-700/50 bg-[#f8f9fa] dark:bg-neutral-900/40 rounded-t-xl min-h-[80px]">
                          <div className="flex items-center gap-5 w-full">
                            <div className="flex items-center shrink-0 pr-4">
                              {activeTabObj?.parentNode ? <button type="button" onClick={() => setActiveDeceasedTab(activeTabObj.parentNode.id === 'root' ? 'root' : activeTabObj.parentNode.personId)} className="flex items-center gap-2 group transition-all"><div className="w-7 h-7 rounded-full border border-[#e9e9e7] dark:border-neutral-700 bg-white dark:bg-neutral-800 flex items-center justify-center text-[#787774] group-hover:text-[#2383e2] group-hover:border-[#2383e2] shadow-sm"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg></div><div className="flex flex-col items-start text-left"><span className="text-[10px] font-bold text-neutral-400 dark:text-neutral-500 leading-none mb-1 uppercase tracking-tight">?곸쐞 ?④퀎</span><div className="flex items-baseline gap-1"><span className="text-[16px] font-black text-neutral-400 dark:text-neutral-500 whitespace-nowrap">{activeTabObj.parentNode.id === 'root' ? (tree.name || '?쇱긽?띿씤') : activeTabObj.parentNode.name}</span><span className="text-[13px] font-bold text-neutral-400 dark:text-neutral-500 whitespace-nowrap">??{getRelStr(currentNode.relation, tree.deathDate)}</span></div></div></button> : <div className="flex items-center px-2"><span className="text-[12px] font-bold text-[#787774] dark:text-neutral-400 tracking-tight">理쒖큹 ?곸냽 ?④퀎</span></div>}
                            </div>
                            <div className="w-px h-8 bg-[#e9e9e7] dark:bg-neutral-700 shrink-0"></div>
                            <div className="flex flex-col justify-center min-w-[80px] max-w-[140px]"><span className="text-[10px] font-bold text-[#1e56a0] dark:text-blue-400 uppercase mb-0.5">{activeDeceasedTab === 'root' ? '?쇱긽?띿씤' : (currentNode.deathDate && tree.deathDate && isBefore(currentNode.deathDate, tree.deathDate) ? '?쇰??듭긽?띿씤' : '?쇱긽?띿씤')}</span><div className="flex items-center overflow-hidden"><span className="text-[16px] font-black text-neutral-800 dark:text-neutral-100 truncate">{getBriefingInfo.name}</span></div></div>
                            <div className="w-px h-8 bg-[#e9e9e7] dark:bg-neutral-700 shrink-0"></div>
                            <div className="flex flex-col justify-center items-center shrink-0"><span className="text-[12px] font-bold text-[#c93f3a] dark:text-red-400 mb-1 leading-none">{currentNode?.deathDate ? `${formatKorDate(currentNode.deathDate)} ?щ쭩` : (tree.deathDate ? `${formatKorDate(tree.deathDate)} ?щ쭩` : '?щ쭩?쇱옄 誘몄긽')}</span><div className="w-[120px] bg-[#fefce8] dark:bg-yellow-900/30 text-[#854d0e] dark:text-yellow-500 border border-[#fef08a] dark:border-yellow-700/50 py-0.5 rounded flex items-center justify-center gap-1 shadow-sm"><span className="text-[9px]">?뽳툘</span><span className="text-[10px] font-black tracking-tighter whitespace-nowrap">{getLawEra(currentNode?.deathDate || tree.deathDate)}??誘쇰쾿</span></div></div>
                            <div className="w-px h-8 bg-[#e9e9e7] dark:bg-neutral-700 shrink-0"></div>
                            <div className="flex flex-col justify-center flex-1 min-w-0 ml-[-10px]"><div className="flex items-baseline gap-2"><span className="text-[10px] font-bold text-neutral-400 dark:text-neutral-500 uppercase">吏遺?/span><span className="text-[17px] font-black text-[#1e56a0] dark:text-blue-400 leading-none">{getBriefingInfo.shareStr}</span></div></div>
                            <div className="flex items-center gap-1.5 ml-auto shrink-0">
                              {!isRootNode && <div className="flex items-center gap-1.5 bg-white dark:bg-neutral-800 border border-[#e9e9e7] dark:border-neutral-700 px-2 py-1 rounded-full shadow-sm"><span className={`text-[11px] font-bold transition-colors select-none cursor-pointer ${!currentNode.isExcluded ? 'text-[#37352f] dark:text-neutral-200' : 'text-[#787774] dark:text-neutral-500'}`} onClick={() => { if (currentNode.isExcluded && (!currentNode.heirs || currentNode.heirs.length === 0)) { alert("?곸냽?몄쓣 癒쇱? ?낅젰?댁＜?몄슂."); return; } const nextVal = !currentNode.isExcluded; handleUpdate(currentNode.id, { isExcluded: nextVal, exclusionOption: nextVal ? '' : 'renounce' }); }}>{currentNode.isExcluded ? '?곸냽沅??놁쓬' : '?곸냽'}</span><button type="button" onClick={() => { if (currentNode.isExcluded && (!currentNode.heirs || currentNode.heirs.length === 0)) { alert("?곸냽?몄쓣 癒쇱? ?낅젰?댁＜?몄슂."); return; } const nextVal = !currentNode.isExcluded; handleUpdate(currentNode.id, { isExcluded: nextVal, exclusionOption: nextVal ? '' : 'renounce' }); }} className={`relative inline-flex h-3.5 w-6 items-center shrink-0 cursor-pointer rounded-full transition-colors duration-200 focus:outline-none ${!currentNode.isExcluded ? 'bg-[#15803d] opacity-80' : 'bg-neutral-200 dark:bg-neutral-600'}`}><span className={`inline-block h-2.5 w-2.5 transform rounded-full bg-white shadow-sm transition duration-200 ${!currentNode.isExcluded ? 'translate-x-2.5' : 'translate-x-0.5'}`} /></button></div>}
                              {canAutoFill && <button type="button" onClick={handleAutoFill} className="text-[11.5px] text-[#37352f] dark:text-neutral-200 font-bold bg-white dark:bg-neutral-800 hover:bg-[#f7f7f5] dark:hover:bg-neutral-700 px-2.5 py-1.5 rounded transition-colors flex items-center border border-[#e9e9e7] dark:border-neutral-700 gap-1.5 shadow-sm" title="?곸쐞 ?④퀎???숈씪???곸냽??紐낅떒??洹몃?濡?媛?몄샃?덈떎"><IconUserGroup className="w-3.5 h-3.5 text-emerald-600" /> 遺덈윭?ㅺ린</button>}
                              <button type="button" onClick={() => { setIsMainQuickActive(!isMainQuickActive); if(!isMainQuickActive) setTimeout(() => document.querySelector('input[placeholder*="?쒓볼踰덉뿉"]')?.focus(), 100); }} className="text-[11.5px] text-[#37352f] dark:text-neutral-200 font-bold bg-white dark:bg-neutral-800 hover:bg-[#f7f7f5] dark:hover:bg-neutral-700 px-2.5 py-1.5 rounded transition-colors flex items-center border border-[#e9e9e7] dark:border-neutral-700 gap-1.5 shadow-sm"><IconUserPlus className="w-3.5 h-3.5 text-[#2383e2]" /> ?곸냽??異붽?</button>
                              <button type="button" onClick={() => { setAiTargetId(activeDeceasedTab); setIsAiModalOpen(true); }} title="?꾩옱 ?곸냽???꾩슜 AI ?섏쐞 ?낅젰" className="flex items-center justify-center w-7 h-7 shrink-0 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 rounded transition-all shadow-sm active:scale-95 ml-1"><span className="text-[14px] leading-none opacity-100 drop-shadow-sm mt-0.5">??/span></button>
                            </div>
                          </div>
                        </div>
                        <div className="px-10 pb-10 pt-6 bg-white dark:bg-neutral-800 rounded-b-xl border border-t-0 border-[#f1f1ef] dark:border-neutral-700/50">
                          {isMainQuickActive && <div className="mb-4 p-4 rounded-lg bg-[#fcfcfb] dark:bg-neutral-800/50 border border-[#e9e9e7] dark:border-neutral-700 animate-in fade-in slide-in-from-top-1 duration-300"><div className="flex flex-col gap-2"><div className="flex items-center justify-between"><div className="text-[12px] font-bold text-[#787774] dark:text-neutral-400">?곸냽???대쫫???쇳몴(,)濡?援щ텇?섏뿬 ?쒓볼踰덉뿉 ?낅젰?섏꽭??/div><button onClick={() => { setIsMainQuickActive(false); setMainQuickVal(''); }} className="text-[#a3a3a3] dark:text-neutral-500 hover:text-[#37352f] dark:hover:text-neutral-300 p-0.5 rounded transition-colors" title="?リ린"><IconX className="w-3.5 h-3.5" /></button></div><div className="flex gap-2"><input autoFocus type="text" value={mainQuickVal} onChange={e => setMainQuickVal(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { handleQuickSubmit(activeDeceasedTab, currentNode, mainQuickVal); setIsMainQuickActive(false); setMainQuickVal(''); } if (e.key === 'Escape') { setIsMainQuickActive(false); setMainQuickVal(''); } }} placeholder="?? ?띻만?? 源泥좎닔, ?댁쁺?? className="flex-1 text-[13px] border border-[#e9e9e7] dark:border-neutral-700 rounded-md px-3 py-1.5 outline-none focus:border-[#d4d4d4] bg-white dark:bg-neutral-900 dark:text-neutral-200 transition-all font-medium text-[#37352f]" /><button onClick={() => { handleQuickSubmit(activeDeceasedTab, currentNode, mainQuickVal); setIsMainQuickActive(false); setMainQuickVal(''); }} className="px-4 py-1.5 bg-white dark:bg-neutral-800 hover:bg-[#efefed] dark:hover:bg-neutral-700 border border-[#e9e9e7] dark:border-neutral-600 text-[#37352f] dark:text-neutral-200 text-[13px] font-bold rounded-md transition-all shadow-sm active:scale-95 whitespace-nowrap">?쇨큵 ?깅줉</button></div></div></div>}
                          {nodeHeirs.length > 0 && <div className="flex items-center px-2 py-2 mb-2 bg-[#fcfcfb] dark:bg-neutral-800/50 rounded-md border border-[#e9e9e7] dark:border-neutral-700 text-[12px] font-bold text-[#787774] dark:text-neutral-400 select-none w-full overflow-hidden"><div className="w-[68px] shrink-0 text-center ml-[10px]"><span className="relative left-[15px]">?곹깭</span></div><div className="w-[72px] shrink-0 text-center ml-[50px]"><span className="relative left-[-20px]">?깅챸</span></div><div className="w-[76px] shrink-0 text-center ml-[30px]"><span className="relative left-[-30px]">愿怨?/span></div><div className="w-[150px] shrink-0 text-center ml-[30px]"><span className="relative left-[-40px]">?щ쭩?щ?/?쇱옄</span></div><div className="w-[180px] shrink-0 text-center ml-[10px] relative"><span className="relative left-[-20px]">?뱀닔議곌굔 (媛媛먯궛)</span></div><div className="w-[180px] shrink-0 text-center ml-[10px] relative"><span className="whitespace-nowrap relative left-[0px] inline-flex items-center">????듭긽??button type="button" title="?꾩옱 ???곸냽???꾩껜 ??젣" onClick={() => { if (!nodeHeirs || nodeHeirs.length === 0) { alert('??젣???곸냽?몄씠 ?놁뒿?덈떎.'); return; } if (window.confirm('?슚 ?꾩옱 ??뿉 ?낅젰??[紐⑤뱺 ?곸냽?????뺣쭚 ??젣?섏떆寃좎뒿?덇퉴?\n(?섏쐞???낅젰???곗씠?곌퉴吏 紐⑤몢 ?④퍡 ??젣?⑸땲??)')) { const targetPId = currentNode.personId; setVault(prev => { if (prev.relationships[targetPId]) prev.relationships[targetPId] = []; return prev; }); } }} className="ml-[50px] p-1 text-neutral-400 hover:text-red-600 dark:hover:text-red-400 rounded-md"><IconTrash2 className="w-3.5 h-3.5" /></button></span></div><div className="w-12 shrink-0 text-center ml-[40px] mr-[10px]"><span className="whitespace-nowrap">??젣</span></div></div>}
                          {nodeHeirs.length === 0 && <div className="flex flex-col items-center justify-center p-8 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-lg text-center gap-2 m-2 mb-4"><span className="text-[#b45309] dark:text-amber-500 font-bold text-[14px]">?좑툘 ?섏쐞 ?곸냽???곗씠?곌? ?놁뒿?덈떎.</span><span className="text-[#787774] dark:text-neutral-400 text-[12.5px]">?곸냽?몄씠 ?녿떎硫??곸쐞 ?곸냽??遺紐??먮뒗 ?뺤젣?먮ℓ)?먭쾶 ?곸냽遺꾩씠 ?먮룞 遺꾨같?⑸땲??</span></div>}
      ...
      {showScrollTop && <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] bg-white/60 dark:bg-neutral-800/60 backdrop-blur-md border border-white/20 dark:border-neutral-700/30 text-[#2383e2] dark:text-blue-400 px-5 py-2.5 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all flex items-center gap-2 text-[13px] font-bold no-print"><span className="text-[16px]">??/span> 留??꾨줈</button>}
        {isAiModalOpen && (() => {
          ...
          const aiPromptText = `?뱀떊? ?쒓뎅 ?곸냽踰뺤뿉 湲곕컲??媛怨꾨룄 遺꾩꽍 諛?JSON ?곗씠??援ъ“???꾨Ц媛?낅땲??
?쒓났??臾몄꽌(?먮뒗 ?대?吏)瑜?遺꾩꽍?섏뿬, ?꾨옒??[?꾧꺽??異붿텧 洹쒖튃]怨?[JSON ?ㅽ궎留??묒떇]??留욎떠 ?꾨꼍??怨꾩링 援ъ“??JSON???앹꽦??二쇱꽭??

[異붿텧 洹쒖튃]
1. 愿怨?relation) 吏?? ?⑥옄??"son", ?ъ옄??"daughter", 諛곗슦?먮뒗 "wife" ?먮뒗 "husband". (1991???댄썑 ?щ쭩嫄댁뿉???먮? ?깅퀎??紐⑤Ⅴ硫?"son"?쇰줈 ?듭씪)
2. ?좎쭨 ?뺤떇: YYYY-MM-DD. (紐낇솗?섏? ?딆쑝硫?"" 鍮?臾몄옄??泥섎━)
3. ?щ쭩 諛??쒖쇅 泥섎━:
   - ?щ쭩?? isDeceased: true, deathDate 湲곗엯.
   - ?곸냽?ш린/?좎궗留?寃곌꺽: isExcluded: true ?ㅼ젙 ?? exclusionOption???ъ쑀 湲곗엯 ("renounce", "predeceased", "lost", "disqualified" 以??앹씪).
4. 援щ쾿(1990???댁쟾 ?щ쭩) 蹂???뺤씤:
   - ?ъ꽦(daughter)??寃쎌슦 議깅낫 臾몃㎘???뚯븙?섏뿬 ?쇱씤?쇱옄(marriageDate)??異쒓? ?щ?(isSameRegister: false)瑜?理쒕????뚯븙?섏뿬 湲곕줉??寃?
   - ?몄＜ ?곸냽?몄씠 紐낆떆??寃쎌슦 isHoju: true 濡??쒖떆.
5. ?ㅼ꽭? 以묒꺽 (?ш???援ъ“):
   - ?먮?媛 ?щ쭩?섏뿬 ??듭긽?띿씠???ъ긽?띿씠 ?쇱뼱?섎뒗 寃쎌슦, ?대떦 ?먮? 媛앹껜??"heirs" 諛곗뿴 ?덉뿉 洹?諛곗슦?먯? ?섏쐞 ?먮??ㅼ쓣 怨꾩냽?댁꽌 以묒꺽(Nesting)?섏뿬 ?꾨꼍???몃━ 援ъ“瑜?留뚮뱾 寃?
6. 怨좎쑀 ?앸퀎??personId) 遺??洹쒖튃 [留ㅼ슦 以묒슂]:
   - 媛??몃Ъ留덈떎 "ai_?쒕뜡臾몄옄?? ?뺥깭??怨좎쑀 personId瑜?遺?ы븷 寃?
   - 臾몃㎘???꾨꼍???숈씪???몃Ъ(?щ윭 媛怨꾩뿉 嫄몄퀜 以묐났 ?깆옣?섎뒗 ??? 諛섎뱶??'?묎컳? personId'瑜?遺?ы븷 寃?
   - ?대쫫留?媛숈? ?숇챸?댁씤(?⑤궓)??寃쎌슦 諛섎뱶??'?쒕줈 ?ㅻⅨ personId'瑜?遺?ы븯??遺꾨━??寃?
7. 紐⑦샇???뺣낫???덈? ?꾩쓽濡?異붿륫?섏? 留먭퀬, JSON 異쒕젰 ???섎떒??吏덈Ц?쇰줈 ?④만 寃?

[JSON ?ㅽ궎留??묒떇 (紐⑤뱺 ?몃뱶??鍮좎쭚?놁씠 ???띿꽦?ㅼ쓣 ?ы븿?댁빞 ??]
{
  "id": "root", // 理쒖긽???쇱긽?띿씤? root, ?섏쐞 ?곸냽?몃뱾? ?꾩쓽??怨좎쑀 ID (?? n_ai_123)
  "name": "留앹씤 ?대쫫",
  "isDeceased": true,
  ...
  "remarriageDate": "",
  "gender": "",
  "personId": "root", // 理쒖긽???쇱긽?띿씤? root, ?섏쐞??ai_?쒕뜡媛?  "relation": "", // ?쇱긽?띿씤? 鍮?臾몄옄?? ?섏쐞??son/daughter/wife ??  "isHoju": false,
  "isExcluded": false,
  "exclusionOption": "",
  "isSameRegister": true,
  "heirs": [
    {
      // ?꾩? ?꾨꼍???숈씪??援ъ“??媛앹껜媛 ?섏쐞 ?곸냽?몄쑝濡쒖꽌 ?ш??곸쑝濡??ㅼ뼱媛?    }
  ]
}`;
          ...
          const handleAiIngest = (input) => {
            ...
            try {
              ...
              const processAiData = (node) => {
                ...
                const isSpouseType = ['wife', 'husband', 'spouse', '泥?, '?⑦렪', '諛곗슦??].includes(node.relation);
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2">
                          <button 
                            onClick={() => { navigator.clipboard.writeText(aiPromptText).then(() => alert('??紐낅졊?닿? ?대┰蹂대뱶??蹂듭궗?섏뿀?듬땲??')); }}
      ...
      {isResetModalOpen && (
        <div className="fixed inset-0 bg-black/40 z-[9999] flex items-center justify-center no-print text-[#37352f] backdrop-blur-[2px] animate-in fade-in duration-200">
          <div className="bg-white dark:bg-neutral-800 p-8 rounded-xl shadow-2xl max-w-sm w-full mx-4 border border-[#e9e9e7] dark:border-neutral-700">
            <h2 className="text-[17px] font-bold mb-2 dark:text-neutral-100">???묒뾽 ?쒖옉</h2>
            <p className="text-[13px] text-[#787774] dark:text-neutral-400 mb-6 font-medium">?꾩옱 吏꾪뻾 以묒씤 ?곗씠?곕? 珥덇린?뷀븯怨?泥섏쓬遺???ㅼ떆 ?쒖옉?좉퉴??</p>
            <div className="flex flex-col gap-2 mt-2">
              <button onClick={() => performReset(true)} className="w-full py-2.5 bg-[#2383e2] hover:bg-[#0073ea] text-white font-bold rounded-lg transition-all text-[13px] shadow-sm">諛깆뾽 ?????珥덇린??/button>
              <button onClick={() => performReset(false)} className="w-full py-2.5 bg-[#ffe2dd] hover:bg-[#ffc1b8] text-[#d44c47] font-bold rounded-lg transition-all text-[13px]">?곗씠????젣 諛?珥덇린??/button>
              <button onClick={() => setIsResetModalOpen(false)} className="w-full py-2.5 mt-2 bg-white dark:bg-neutral-800 border border-[#d4d4d4] dark:border-neutral-600 text-[#37352f] dark:text-neutral-300 font-bold rounded-lg transition-all text-[13px] hover:bg-neutral-50 dark:hover:bg-neutral-700">痍⑥냼</button>
            </div>
          </div>
        </div>
      )}
