...
const HeirRow = ({ node, finalShares, handleUpdate, removeHeir, inheritedDate, rootDeathDate, onKeyDown, rootIsHoju, onTabClick, parentNode }) => {
  ...
  // ?뮕 ?ы샎?쇰줈 ?명븳 ?ㅼ쐞移?媛뺤젣 ON 諛⑹????щ씪?대뱶 寃쎄퀬李??곹깭
      ...
      <div className="ml-[20px] shrink-0 flex items-center">
        <button
          ...
          onClick={() => {
            ...
            // ?슚 ?ㅼ쐞移섎? 耳쒕젮怨?OFF -> ON) ?섎뒗?????媛쒖떆 ???ы샎?먮씪硫?
            if (node.isExcluded && isPreRemarried) {
              setShowRemarriageWarning(true); // ?щ씪?대뱶 李??ㅽ뵂!
              return; // ?ㅼ쐞移섍? 耳쒖???寃껋쓣 ?먯쿇 李⑤떒
            }
            ...
            setShowRemarriageWarning(false); // ?뺤긽 ?묐룞 ??寃쎄퀬李??リ린
          }}
                ...
                {showMarriedDaughter && (
                  <div className="flex items-center gap-1.5">
                    <div onClick={() => handleUpdate(node.id, 'isSameRegister', node.isSameRegister === false ? true : false)} className="relative flex items-center w-[64px] h-[26px] bg-[#efefed] dark:bg-neutral-900 rounded-full border border-[#e5e5e5] dark:border-neutral-700 p-0.5 cursor-pointer select-none">
                      ...
                      <div className={`flex-1 text-center z-10 text-[11px] font-bold ${node.isSameRegister === false ? 'text-[#37352f]' : 'text-[#a3a3a3]'}`}>異쒓?</div>
                    </div>
                    {(() => {
                      let m = '';
          ...
          <div className="modal-nav-area">
            ...
            {node.relation === 'daughter' && (
              <div className="mb-6 p-4 bg-white dark:bg-neutral-800 border border-[#e5e5e5] dark:border-neutral-700 rounded-lg relative">
                ...
                <div className="space-y-3 mt-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-[13px] font-bold text-[#504f4c]">?쇱씤 ?쇱옄</label>
                    <DateInput 
                      autoFocus
