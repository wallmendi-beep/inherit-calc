...
export const buildTreeFromVault = (vault) => {
  ...
  const buildNode = (personId, parentDeathDate = null, visited = new Set()) => {
    ...
    links.forEach(link => {
      // ???먯떇???щ쭩?쇱씠 ??遺紐?蹂대떎 ??쑝硫??먯떇 ?щ쭩??湲곗?, ?꾨땲硫????щ쭩??湲곗? ?좎?
...
const MiniTreeView = ({ node, level = 0, onSelectNode, visitedHeirs = new Set(), deathDate, toggleSignal, searchQuery, matchIds, currentMatchId, guideStatusMap }) => {
  ...
  // ?뮕 ?ㅻ쭏??媛?대뱶 ?곹깭 留?guideStatusMap) 100% ?좊ː 濡쒖쭅?쇰줈 ?꾨㈃ 援먯껜!
  const status = guideStatusMap?.[node.id] || guideStatusMap?.[node.name] || {};

  // ?먯떇 ?대뜑??以묒뿉 ?먮윭(?슚)??沅뚭퀬(?뮕)媛 ?⑥뼱?덈뒗吏 ?낆깄???ㅼ틪?⑸땲??
  ...
  // ?꾩옱 ?몃뱶媛 ?먮윭嫄곕굹, ?대뜑媛 ?묓??덈뒗???먯떇 以묒뿉 ?먮윭媛 ?덉쑝硫??슚
  const showMandatory = status.mandatory || (!isExpanded && childStatus.hasMan);
  // ?꾩옱 ?몃뱶媛 沅뚭퀬嫄곕굹, ?대뜑媛 ?묓??덈뒗???먯떇 以묒뿉 沅뚭퀬媛 ?덉쑝硫??뮕 (?? ?먮윭媛 ?놁쓣 ?뚮쭔)
...
function App() {
  ...
  // ?뮕 AI 留덈쾿??紐⑤떖 ?곹깭 愿由?  const [isAiModalOpen, setIsAiModalOpen] = useState(false);
  const [aiInputText, setAiInputText] = useState("");
  const [aiTargetId, setAiTargetId] = useState('root'); // ?뮕 AI媛 ?곗씠?곕? 苑귥븘?ｌ쓣 ?寃??꾩튂 湲곗뼲!
  const [showQrCode, setShowQrCode] = useState(false); // ?뮕 QR 肄붾뱶 ?쒖떆 ?щ?
  ...
  // ?뮕 [v3.0] ?듭떆?붿뼵 蹂쇳듃(Vault) 湲곕컲 History ?곹깭 愿由?  const [vaultState, setVaultState] = useState({
  ...
  // ?뮕 [v3.0] Vault 吏곸젒 ?쒖뼱 ?붿쭊 (?⑥씪 吏꾩떎 怨듦툒??
  ...
  // ?슚 [v3.0] 留덈쾿???꾨줉??(援щ쾭???꾨꼍 ?명솚??諛⑹뼱留?
  // ?붾㈃?먯꽌 湲곗〈 肄붾뱶媛 ?몃━瑜?議곗옉?섎젮 ?섎㈃, ?뚯븘???듭떆?붿뼵 留곹겕濡??뺤텞 蹂?섑빐????ν빐 以띾땲??
  ...
  // ?뮕 [v3.0] ?붾㈃(UI)??肉뚮젮二쇰뒗 ?몃━??以묒븰 李쎄퀬?먯꽌 0.001珥?留뚯뿉 議곕┰?댁꽌 ?쒓났
  ...
  const handleNavigate = (nodeId) => {
    ...
    // ?뮕 ?뱀닔 紐낅졊?? "?대떦 ?곸냽?몄쓽 ??諛? ?덉쑝濡??ㅼ씠?됲듃 吏꾩엯?대씪!"
    ...
    const findTabIdForNode = (currentNode, currentTabId) => {
      // 猷⑦듃(?쇱긽?띿씤) 蹂몄씤?닿굅??理쒖큹 吏꾩엯?먯씪 寃쎌슦
      ...
      if (currentNode.heirs) {
        for (const h of currentNode.heirs) {
          // ?슚 ?듭떖 ?쎌뒪: ?먯떇(h)???곕━媛 李얜뒗 ??곸씠?쇰㈃, ???щ엺??'?낅젰移?Row)'???붾㈃???섏삤??怨녹? 蹂몄씤????씠 ?꾨땲??'遺紐⑥쓽 ??currentTabId)'?낅땲??
          if (h.id === nodeId || h.personId === nodeId) return currentTabId;

          // 紐?李얠븯?쇰㈃ ?먯떇????쑝濡?怨꾩냽 ?뚭퀬??땲??
  ...
  // ?뮕 [?낃렇?덉씠?? ?ㅼ떆媛?遺꾩떊(Clone) ?숆린???붿쭊???묒옱??handleUpdate
  const handleUpdate = (id, changes, value) => {
    ...
    // ?뮕 ?덈줈??湲곕뒫: 遺紐?愿怨??깅퀎) 蹂寃????섏쐞 諛곗슦??愿怨??먮룞 ?ㅼ쐞移?
    if (field === 'relation') {
    ...
    // ?뮕 [v3.0] ?듭떆?붿뼵 ?숆린?? 以묒븰 李쎄퀬 ?섎굹留??섏젙?섎㈃ ?곗＜??紐⑤뱺 ??씠 0.001珥덈쭔???숆린?붾맖!
    setVault(prev => {
      ...
      // 2. 遺紐??먯떇 媛꾩쓽 愿怨??ㅼ쐞移??섏젙 (?꾩옱 遺紐???뿉?쒕쭔 諛섏쁺)
  ...
  // ?뮕 [v3.0] ?곸냽??異붽? (?щ엺 1紐?留뚮뱾怨? 留곹겕 1以?湲뗪린 ??)
  const addHeir = (parentId) => {
    ...
    setVault(prev => {
      ...
      // ?먮?媛 ?앷린硫?遺紐⑥쓽 ?곸냽沅??ㅼ쐞移??먮룞 ?뺤긽??      if (parentPersonId !== prev.meta.rootPersonId) {
  ...
  // ?뮕 [v3.0] ?곸냽????젣 (議댁옱瑜?吏?곕뒗 寃??꾨땲???곌껐?좊쭔 ?밸몣 ?먮쫫!)
  const removeHeir = (id) => {
    let targetPersonId = id;
    let parentPersonId = null;
    ...
    setVault(prev => {
      // ?대떦 遺紐⑥쓽 紐낅떒?먯꽌 ?먯떇怨쇱쓽 ?곌껐 留곹겕留?吏?뚮쾭由쎈땲?? (?ㅻⅨ ??臾댁궗??
  ...
  const { finalShares, calcSteps, warnings = [] } = useMemo(() => {
    // ?뮕 ?ъ슜?먮떂 吏?쒕줈 ?꾩깮??'吏꾩쭨 2/3?쒖쐞 ?먮룞 二쇱엯 ?붿쭊'
    const preprocessTree = (n, parentDate, parentNode) => {
      ...
      if (clone.id !== 'root' && !clone.isExcluded) {
        ...
        // ?슚 ?ъ긽???꾩궗留??몃뜲 ?먮?媛 鍮꾩뼱?덈떎硫? 湲곌퀎媛 ?뚯븘??遺紐??뺤젣瑜?李얠븘?듬땲??
          ...
          if (!isSpouseType) {
            ...
            // 2?쒖쐞: ?щ쭩 ?뱀떆 ?댁븘?덈뒗 吏곴퀎議댁냽(?대㉧???꾨쾭吏) 李얘린
            ...
            // ?뮕 [諛곗슦???꾩슜 留덈쾿] ?놁뿉 ?덈뒗 ?⑦렪/?꾨궡???먯떇?ㅼ쓣 ???먯떇?쇰줈 100% ?먮룞 蹂듭궗!
  ...
  const multipleSpouseGuides = useMemo(() => {
    ...
    const checkSpouses = (node) => {
      ...
      if (spouses.length > 1) {
        guides.push({
          id: `tab:${node.personId}`, // ?렞 ?쎌뒪 1: 遺紐???씠 ?꾨땶 臾몄젣瑜??닿껐??'?????쇰줈 0.1珥?留뚯뿉 吏곹뻾!
          uniqueKey: `multi-spouse-${node.personId}`, 
          targetTabId: node.personId, 
          type: 'mandatory',
          text: `[${node.name || '?대쫫?놁쓬'}]?섏쓽 諛곗슦?먭? 2紐??댁긽?낅땲?? ?좏슚??諛곗슦??1紐낆쓣 ?쒖쇅?섍퀬 ?섎㉧吏???ㅼ쐞移섎? 議곗옉???쒖쇅 泥섎━??二쇱꽭??`
        });
      }
  ...
  const hojuMissingGuides = useMemo(() => {
    ...
    const checkHoju = (node) => {
      if (node.isDeceased && node.heirs && node.heirs.length > 0) {
        // ?슚 ?쎌뒪 2: ?쇱긽?띿씤???ъ꽦(?? 泥?????寃쎌슦 媛?대뱶媛 ?몄＜ 吏?뺤쓣 媛뺤슂?섏? ?딆뒿?덈떎!
        const isFemale = ['wife', 'daughter', 'mother', 'sister', '泥?, '諛곗슦??].includes(node.relation) || node.gender === 'female';
        
        // ?⑥꽦?닿굅?? ?덉쇅?곸쑝濡?蹂몄씤??吏곸젒 ?몄＜ 吏?꾨? 媛吏怨??덉뿀??寃쎌슦?먮쭔 ?몄＜ ?꾨씫??寃?ы빀?덈떎.
        if (!isFemale || node.isHoju) {
          ...
          if (needsHoju && !hasHoju) {
            guides.push({
              id: `tab:${node.personId}`, // ?렞 ?쎌뒪 1: '?????쇰줈 ?ㅼ씠?됲듃 ?먰봽
              uniqueKey: `missing-hoju-${node.personId}`,
              targetTabId: node.personId, 
              type: 'mandatory',
              text: `[${node.name || '?대쫫?놁쓬'}]?섏쓽 ?곸냽??以??몄＜?곸냽?몄씠 ?놁뒿?덈떎. ?몄＜ ?ㅼ쐞移섎? 耳쒖＜?몄슂.`
            });
          }
        }
      }
  ...
  // ?뮕 [?쇰━ 遺덉씪移?媛먯? ?쇱꽌: ?좎쭨 ?곗씠??vs UI ?ㅼ쐞移??곹깭]
      ...
      if (node.relation === 'daughter' && node.marriageDate && effectiveDate) {
        ...
        if (isMarriedBeforeDeath && node.isSameRegister !== false) {
          guides.push({
            ...
            targetTabId: parentPersonId, // ?렞 ???ㅼ쐞移섎? ?꾨뒗 嫄곕땲源??섎? ?ы븿?섍퀬 ?덈뒗 '遺紐????쇰줈 ?대룞!
            type: 'mandatory',
            text: `[${node.name || '?대쫫?놁쓬'}]?섏쓽 ?쇱씤?쇱옄(${node.marriageDate})媛 ?곸냽媛쒖떆??${effectiveDate})蹂대떎 鍮좊Ⅴ吏留? ?ㅼ쐞移섍? [?숈씪]濡??섏뼱 ?덉뒿?덈떎. ?뺤씤 ??[異쒓?]濡?蹂寃쏀빐 二쇱꽭??`
          });
        }
      }
      ...
      if (node.deathDate && effectiveDate && isBefore(node.deathDate, effectiveDate) && !isSpouse) {
        if (!node.isExcluded) {
          guides.push({
            ...
            targetTabId: parentPersonId, // ?렞 留덉갔媛吏濡?遺紐???쑝濡??대룞!
            type: 'mandatory',
            text: `[${node.name || '?대쫫?놁쓬'}]?섏쓽 ?щ쭩?쇱옄(${node.deathDate})媛 ?곸냽媛쒖떆??${effectiveDate})蹂대떎 鍮좊쫭?덈떎. ??듭긽??泥섎━瑜??꾪빐 ?ㅼ쐞移섎? 議곗옉?섏뿬 [?곸냽沅??놁쓬/??듭긽???쇰줈 蹂寃쏀빐 二쇱꽭??`
          });
        }
      }
  ...
  // ?슚 [?듭떖 ?쎌뒪: 遺꾩떊(Clone) 以묐났 寃쎄퀬 ?꾨꼍 ?쒓굅]
  ...
  rawSmartGuides.forEach(g => {
    const key = g.uniqueKey || g.text; // ?먯껜 怨좎쑀??personId 湲곕컲)媛 ?덉쑝硫?洹멸구 ?곌퀬, ?놁쑝硫??띿뒪???댁슜?쇰줈 嫄곕쫫
                ...
                <div className="text-[13px] font-bold text-[#504f4c] dark:text-neutral-300 pointer-events-none animate-in fade-in slide-in-from-top-1 duration-200">
                  ...
                  {activeTab === 'input' && warnings.map((w, i) => (
                    <div 
                      key={`w-${i}`} 
                      onClick={() => {
                        // ?뮕 1?쒖쐞: 媛?대뱶媛 '??臾몄젣瑜??닿껐??理쒖쟻????targetTabId)'??吏?뺥뻽?ㅻ㈃ 臾댁“嫄?洹멸납?쇰줈 吏곹뻾!
                        if (w.targetTabId && deceasedTabs.some(t => t.id === w.targetTabId)) {
                          setActiveDeceasedTab(w.targetTabId);
                        } 
                        // 2?쒖쐞: 吏?뺣맂 紐⑹쟻吏媛 ?녿떎硫? 湲곗〈 諛⑹떇?濡??대떦 ?몃뱶媛 ?꾩튂??遺紐???쓣 ??텛?곹빐???대룞
                        else if (w.id) {
                      ...
                      title={w.id ? "?대┃ ???섏젙???꾪빐 ?대떦 ??쑝濡??대룞?⑸땲?? : ""}
                    >
                      <span className="mt-0.5">?슚</span>
                  ...
                  {activeTab === 'input' && smartGuides.filter(g => g.type === 'mandatory').map((g, i) => (
                    <button 
                      ...
                      onMouseDown={(e) => e.stopPropagation()}
                      onClick={() => {
                        // ?뮕 ?붿쭊??怨꾩궛?댁? 紐⑹쟻吏(targetTabId)媛 ?덈떎硫?臾댁“嫄?嫄곌린濡?吏곹뻾?⑸땲??
                        if (g.targetTabId) {
                          if (g.targetTabId === tree.personId) {
                            setActiveDeceasedTab('root'); // 源?곸“ ??理쒖긽???쇱긽?띿씤??寃쎌슦
                          } else {
                            setActiveDeceasedTab(g.targetTabId); // 洹???紐⑤뱺 ??                          }
                        }
                      }}
        ...
        <div className="w-[1080px] min-w-[1080px] shrink-0 px-6 flex items-center justify-between h-full flex-nowrap">
          <div className="flex items-center gap-3 flex-nowrap shrink-0">
            ...
            <div className="flex items-center gap-2 whitespace-nowrap shrink-0 overflow-visible">
              <div className="flex items-center text-[#37352f] dark:text-neutral-100 font-bold text-[18px] tracking-tight whitespace-nowrap shrink-0"><IconCalculator className="w-5 h-5 mr-1.5 text-[#787774] dark:text-neutral-400 shrink-0" />?곸냽吏遺?怨꾩궛湲?PRO <span className="ml-1.5 text-[11px] font-medium bg-[#e9e9e7] dark:bg-neutral-700 px-1.5 py-0.5 rounded text-[#787774] dark:text-neutral-400 shrink-0">v3.0.0</span></div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {/* ?뮕 AI 媛怨꾨룄 留덈쾿??踰꾪듉 (?뚰봽???뚯뒪??+ ?ъ씠利??듭씪) */}
            <button 
              onClick={() => { setAiTargetId('root'); setIsAiModalOpen(true); }}
              title="媛怨꾨룄 ?꾩껜 AI ?먮룞?낅젰" 
            ...
            {/* ?㎛ ?ㅻ쭏??媛?대뱶 ?몄텧 踰꾪듉 (AI 踰꾪듉怨??됱긽 諛??뚮쭏 ?꾨꼍 ?듭씪) */}
            <button 
            ...
            <button onClick={undoTree} disabled={vaultState.currentIndex <= 0} className="disabled:opacity-40 disabled:cursor-not-allowed text-[#787774] hover:text-[#37352f] hover:bg-[#efefed] px-2 py-1 rounded border border-transparent hover:border-[#d4d4d4] text-[12px] font-bold transition-colors flex items-center gap-1">
                        ...
                        <div className="flex items-center shrink-0 pr-4">
                          {activeTabObj && activeTabObj.parentNode && activeDeceasedTab !== 'root' ? (
                            <button
                              type="button"
                              onClick={() => {
                                // ?뮕 怨좎튇 遺遺? 猿띾뜲湲?id媛 ?꾨땲??吏꾩쭨 DNA??personId濡??섍꺼???뺤긽?곸쑝濡???쓣 李얠뒿?덈떎!
                    ...
                    <div className="px-10 pb-10 pt-6 bg-white dark:bg-neutral-800 rounded-b-xl border border-t-0 border-[#f1f1ef] dark:border-neutral-700/50">
                      ...
                      {nodeHeirs.length > 0 && (
                        <div className="flex items-center px-2 py-2 mb-2 bg-[#fcfcfb] dark:bg-neutral-800/50 rounded-md border border-[#e9e9e7] dark:border-neutral-700 text-[12px] font-bold text-[#787774] dark:text-neutral-400 select-none animate-in fade-in duration-300 w-full overflow-hidden">
                          ...
                          <div className="w-[76px] shrink-0 text-center ml-[30px]"><span className="relative left-[-30px]">愿怨?/span></div>
                          ...
                          <div className="w-[180px] shrink-0 text-center ml-[10px] relative">
                            <span className="whitespace-nowrap relative left-[0px] inline-flex items-center">
                              ????듭긽??                              {/* ?뿊截??곸냽???꾩껜 ??젣 踰꾪듉 (湲?⑤줈遺???뺥솗??50px ?ㅻⅨ履?諛곗튂) */}
                              <button
                                type="button"
                                title="?꾩옱 ???곸냽???꾩껜 ??젣"
                                onClick={() => {
                                  ...
                                  if (window.confirm('?슚 ?꾩옱 ??뿉 ?낅젰??[紐⑤뱺 ?곸냽?????뺣쭚 ??젣?섏떆寃좎뒿?덇퉴?\n(?섏쐞???낅젰???곗씠?곌퉴吏 紐⑤몢 ?④퍡 ??젣?⑸땲??)')) {
                                    // ?뮕 [v3.0] 蹂쇳듃 吏곸젒 ?寃? ?대떦 ??二쇱씤??'?먮? 留곹겕 紐낅떒'???꾩쟾??鍮꾩썙踰꾨┰?덈떎.
              ...
              <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 py-4 border-b border-gray-200 dark:border-neutral-700 flex justify-between items-center bg-indigo-50 dark:bg-indigo-900/30">
                  <h2 className="text-lg font-bold text-indigo-700 dark:text-indigo-300 flex items-center gap-2">
                    <span>??/span> {targetName} AI ?먮룞 ?낅젰湲?<span className="text-sm font-medium opacity-70 ml-1">(?쒖쟻?깅낯 / 媛怨꾨룄)</span>
                  </h2>
                ...
                <div className="p-6 overflow-y-auto flex-1">
                  ...
                  <div className="mb-6 p-4 bg-gray-50 dark:bg-neutral-700/50 rounded-lg border border-gray-200 dark:border-neutral-600">
                    <h3 className="font-bold text-gray-800 dark:text-gray-200 mb-2">1?④퀎: 紐낅졊??蹂듭궗?섍린</h3>
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-3 leading-relaxed">
                      ?꾨옒 踰꾪듉???뚮윭 紐낅졊?대? 蹂듭궗???? <b>琉쇳듉, ChatGPT, ?대줈?? ?쒕??섏씠</b> ?깆뿉 臾몄꽌 ?ъ쭊怨??④퍡 遺숈뿬?ｌ쑝?몄슂.<br/>
                      <span className="text-indigo-600 dark:text-indigo-400 font-medium">??怨듬Ц???쒖쟻?깅낯, 媛議깃?怨꾩쬆紐낆꽌)??臾쇰줎, ?좎껌?몄씠 吏곸젒 ?먯쑝濡?洹몃┛ 媛怨꾨룄 ???щЦ???ъ쭊???꾨꼍?섍쾶 ?몄떇?⑸땲??</span>
                    </p>
                    
                    {/* ?뮕 v3.0 ?꾨＼?꾪듃 蹂듭궗 諛?QR ?꾩넚 ?곸뿭 */}
                    {(() => {
                      const aiPromptText = `泥⑤???臾몄꽌(?쒖쟻?깅낯, 媛議깃?怨꾩쬆紐낆꽌, 媛怨꾨룄 硫붾え ?? ?ъ쭊??蹂닿퀬, ?꾨옒 JSON ?묒떇??留욎떠 媛議?愿怨꾨? 異붿텧?댁쨾.

[洹쒖튃]
1. 以묒떖 ?몃Ъ(留앹씤)??湲곗??쇰줈 ?⑥옄??"son", ?ъ옄??"daughter", 諛곗슦?먮뒗 "wife" ?먮뒗 "husband"濡??묒꽦??
2. ?щ쭩?먮뒗 isDeceased瑜?true濡??섍퀬, deathDate瑜?"YYYY-MM-DD" ?뺥깭濡??ｌ뼱.
3. ?슚 [以묒슂] ?쒖쟻?깅낯???꾩쿂, ?꾩쿂 ??諛곗슦?먭? ?щ윭 紐?湲곗옱?섏뼱 ?덈떎硫??꾩쓽濡??먮떒/??젣?섏? 留먭퀬 臾몄꽌???덈뒗 ?濡??쇰떒 ?꾨? ???낅젰??
4. 臾몄꽌??異쒓????쇱씤?????덉쑝硫?marriageDate?? ?ы샎?쇱씠 ?덉쑝硫?remarriageDate??"YYYY-MM-DD" ?뺥깭濡?湲곗옱??
5. ?먮??ㅼ? 諛섎뱶???대떦 遺紐⑥쓽 heirs 諛곗뿴 ?덉뿉 ?뺥솗???ｌ뼱.
6. ?묐떟? 臾댁“嫄?JSON 肄붾뱶 釉붾줉?쇰줈留?異쒕젰?? ?ㅻⅨ ?ㅻ챸? ?덈? ?섏?留?

[異쒕젰 ?덉떆]
{
  "name": "?띻만??, "isDeceased": true, "deathDate": "1980-01-01",
  "heirs": [
    { "name": "?닿컩??, "relation": "wife", "isDeceased": true, "deathDate": "1975-01-01" },
    { "name": "諛뺤쓣?", "relation": "wife", "remarriageDate": "1985-05-05" },
    { "name": "?띾컮??, "relation": "daughter", "marriageDate": "1995-10-20" }
  ]
}`;
                      return (
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={() => {
                                navigator.clipboard.writeText(aiPromptText).then(() => alert('??紐낅졊?닿? 蹂듭궗?섏뿀?듬땲?? ?곗떆??AI ?깆뿉 ?ъ쭊怨??④퍡 遺숈뿬?ｌ쑝?몄슂.'));
                              }}
                            ...
                            >
                              {showQrCode ? '?솃 QR ?④린湲? : '?벑 ?ㅻ쭏?명룿 QR ?꾩넚'}
                            </button>
                          </div>

                          {/* ?뮕 ?ㅽ봽?쇱씤 QR 肄붾뱶 ?뚮뜑留??곸뿭 */}
                          {showQrCode && (
                            <div className="mt-4 flex flex-col items-center justify-center p-6 bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-lg shadow-inner">
                              ...
                              <p className="mt-4 text-sm text-gray-600 dark:text-gray-300 font-medium text-center leading-relaxed">
                                ?ㅻ쭏?명룿 湲곕낯 移대찓?쇰줈 ?ㅼ틪?섏꽭??<br/>
                                <span className="text-blue-500 font-bold">?명꽣???곌껐 ?놁씠??/span> ?꾨＼?꾪듃媛 蹂듭궗?⑸땲??
                              </p>
                            </div>
                          )}
                        </div>
                  ...
                  {/* 2?④퀎: 遺숈뿬?ｊ린 ?곸뿭 */}
                  <div>
                    <h3 className="font-bold text-gray-800 dark:text-gray-200 mb-2">2?④퀎: 寃곌낵 ?곗씠??遺숈뿬?ｊ린</h3>
                    <textarea 
                      value={aiInputText}
                      onChange={(e) => setAiInputText(e.target.value)}
                      placeholder="AI媛 留뚮뱾?댁? { ... } 濡??쒖옉?섎뒗 肄붾뱶瑜??닿납??洹몃?濡?遺숈뿬?ｌ쑝?몄슂."
                ...
                <div className="p-4 border-t border-gray-200 dark:border-neutral-700 flex justify-end gap-2 bg-gray-50 dark:bg-neutral-800/50">
                  <button onClick={() => setIsAiModalOpen(false)} className="px-4 py-2 text-gray-600 font-medium hover:bg-gray-200 rounded-md transition-colors">痍⑥냼</button>
                  <button 
                    onClick={() => {
                      try {
                        ...
                        // ?뮕 [?곗씠???꾩쿂由? 諛붿퐫??諛쒓툒 諛?紐낅갚??援ъ“???섎룄(??듭긽?? 諛섏쁺
                        const processAiData = (node) => {
                          ...
                          // ?슚 ?먯떇(??듭긽?띿씤)??媛숈씠 ?ㅼ뼱?붾떎??嫄???듭긽?띿씠?쇰뒗 紐낅갚???섎룄! 
                          // 諛붾낫媛숈씠 媛?대뱶濡?臾살? 留먭퀬 ?뚯븘???ㅼ쐞移섎? [??듭긽???쇰줈 ?명똿?댁쨳?덈떎.
                          if (node.heirs && node.heirs.length > 0) {
                        ...
                        // ?렞 ?寃잛씠 'root(硫붿씤)'硫??꾩껜 ?롮뼱?곌린, ?꾨땲硫??뱀젙 諛??덉뿉 ??
                        if (aiTargetId === 'root') {
                          ...
                          setTree(prev => {
                            const injectHeirs = (n) => {
                              if (targetRawIds.includes(n.id)) {
                                ...
                                // ?슚 ?뱀젙 ??뿉 ?먯떇???덈줈 苑귥븘?ｌ뿀?ㅻ㈃ 洹???二쇱씤???ㅼ쐞移섎? ?뚯븘??爰쇱쨳?덈떎!
                        ...
                        setAiInputText(""); 
                        alert(`???깃났?곸쑝濡?${aiTargetId === 'root' ? '媛怨꾨룄媛' : '?곸냽?몄씠'} ?먮룞 ?낅젰?섏뿀?듬땲??\n?쇱そ??[?ㅻ쭏??媛?대뱶]瑜??뺤씤?섎ŉ 寃?섑빐 二쇱꽭??`);
                      } catch (error) {
                        alert("?슚 ?곗씠???뺤떇???섎せ?섏뿀?듬땲?? AI媛 留뚮뱾?댁? JSON ?띿뒪?멸? 留욌뒗吏 ?ㅼ떆 ?쒕쾲 ?뺤씤?댁＜?몄슂.");
                      }
                    }}
