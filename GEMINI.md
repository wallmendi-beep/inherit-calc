...
## 🌿 브랜치 관리 및 배포 버전 규정
- **main (기본):** 고해상도 그래픽, 최신 Tauri 앱 환경, ESNext 모듈 방식 지향.
- **feature/legacy-offline (호환성):** 구형 브라우저 엔진(회사 환경), 인터넷 단절(오프라인) 전용. IIFE 빌드 방식.
- **배포 방식:** 구형 환경 요청 시 `legacy-offline` 브랜치에서 추출한 HTML을 제공합니다.
...
- 현재 버전: v2.0.7 (Main: Tauri Optimized / Legacy: Offline HTML)
