...
## 상속인 컬럼 배치 규정 (v1.8.6 정밀 수정)

### 순차적 정방향 배치 (왼쪽 0px 기준)
0. **드래그 핸들**: 시작점 마진 **10px** (너비 20px)
1. **상태 토글**: 핸들에서 **20px** 지점 (너비 28px)
2. **성명 입력**: 토글에서 **50px** 지점 (너비 96px)
3. **관계 선택**: 성명에서 **30px** 지점 (너비 96px)
4. **사망일자**: 관계에서 **50px** 지점 (너비 150px)
5. **특수조건**: 사망일자에서 **50px** 지점 (**너비 180px**)
6. **재/대습상속**: 특수조건에서 **50px** 지점 (너비 112px)
7. **삭제 버튼**: 재상속 버튼에서 **20px** 지점 (너비 48px)
