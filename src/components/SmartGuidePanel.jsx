  ...
  return (
    <ContextualDrawer isOpen={showNavigator} onClose={() => setShowNavigator(false)} title="스마트 가이드">
      <div className="space-y-6 p-5">
              ...
              {visibleAuditActionItems.map((item, index) => (
                <li key={`audit-${item.id || index}`}>
                  <button
                    ...
                    className="w-full rounded-lg border border-red-100 bg-red-50/70 p-3 text-left transition-all hover:bg-red-50 dark:border-red-900/30 dark:bg-red-950/20"
                  >
          ...
          <section className="space-y-3">
            ...
            <ul className="space-y-2">
              {warnings.map((warning, index) => (
                <li key={`warning-${index}`} className="rounded-lg border border-amber-100 bg-amber-50/50 p-3 dark:border-amber-900/30 dark:bg-amber-950/20">
                  <div className="flex items-start justify-between gap-2">
                    <span className="text-sm leading-snug text-slate-700 dark:text-neutral-200">{warning.text || warning}</span>
                    <WarningBadge>확인</WarningBadge>
                  </div>
                </li>
              ))}
            </ul>
              ...
              {mandatoryGuides.map((guide, index) => (
                <li key={`mandatory-${index}`}>
                  <button
                    ...
                    className="w-full rounded-lg border border-slate-200 bg-white p-3 text-left shadow-sm transition-all hover:bg-slate-50 dark:border-neutral-700 dark:bg-neutral-800/50 dark:hover:bg-neutral-800"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <span className="text-[12.5px] font-bold leading-relaxed text-slate-800 dark:text-neutral-100">{guide.text}</span>
                      <FatalBadge>필수</FatalBadge>
                    </div>
                  </button>
                </li>
              ))}
            ...
            <ul className="space-y-2">
              {recommendedGuides.map((guide, index) => (
                <li key={`recommended-${index}`} className="group relative">
                  <button
                    onClick={() => handleNavigate(guide.targetTabId || guide.id)}
                    className="w-full rounded-lg border border-slate-200 bg-white p-3 pr-10 text-left shadow-sm transition-all hover:bg-slate-50 dark:border-neutral-700 dark:bg-neutral-800/40 dark:hover:bg-neutral-800"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <span className="text-[12.5px] leading-relaxed text-slate-700 dark:text-neutral-200">{guide.text}</span>
                      <WarningBadge>권장</WarningBadge>
                    </div>
                  </button>
                  <button
                    onClick={(e) => {
              ...
              {globalMismatchReasons.map((reason, index) => (
                <li key={`global-${index}`}>
                  <button
                    ...
                    className="w-full rounded-xl bg-slate-900 p-4 text-left text-white shadow-lg transition-all hover:scale-[1.01] dark:bg-neutral-700"
                  >
              ...
              {repairHints.map((hint, index) => (
                <li key={`hint-${index}`}>
                  <button
                    ...
                    className="w-full rounded-xl border border-slate-200 bg-slate-50/70 p-3 text-left transition-all hover:bg-white dark:border-neutral-700 dark:bg-neutral-900/40 dark:hover:bg-neutral-800"
                  >
        ...
        {showAutoCalcNotice && (
          <section className="mt-4 rounded-xl border border-blue-100 bg-blue-50/30 p-4 dark:border-blue-900/20 dark:bg-blue-900/10">
