...
export const MultiplierBadge = ({ multiplier }) => {
  ...
  const baseClass = "px-1.5 py-0.5 rounded-[3px] border text-[10px] font-normal shadow-sm whitespace-nowrap bg-white ml-1";
...
export const BadgeToggle = ({ 
  ...
  return (
    <button
      ...
      className={`
        px-3 py-1 rounded-full border text-[11px] font-normal transition-all duration-200 active:scale-95 shadow-sm
...
export const ExclusionBadge = ({ label, onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className="px-3 py-1 rounded-full border border-rose-200 bg-rose-50 text-rose-700 text-[11px] font-normal hover:bg-rose-100 transition-colors shadow-sm"
  >
    {label}
  </button>
);
